/// Filters, sorting, and pagination settings for the NPS topics endpoint.
///
...
///
/// Park codes narrow the returned topics to those related to the requested parks. The live
/// endpoint ignores an identifier it does not recognize rather than matching nothing.
///
/// ```swift
/// let query = try ParkTopicQuery(parkCodes: [ParkCode("mamc")], sort: [.descending("name")])
/// let endpoint = Endpoint.parkTopics(query: query)
/// ```
public struct ParkTopicQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkTopic`` values.
  public typealias Item = ParkTopic

  /// The topics collection path.
  public static let path = "/topics"
  ...
  /// The topics parameters for this page, omitting empty identifier, code, and sort arrays.

/// Filters, sorting, and pagination settings for the NPS things to do endpoint.
///
...
/// maximum is assumed.
///
/// NPS documents `relevanceScore` as the only sort field for things to do; without a sort, the
/// provider orders by date last modified, newest first. The live service rejects other fields,
...
/// let endpoint = Endpoint.thingsToDo(query: query)
/// ```
public struct ThingToDoQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ThingToDo`` values.
  public typealias Item = ThingToDo

  /// The things to do collection path.
  public static let path = "/thingstodo"

  /// Thing to do identifiers in caller-supplied order; empty means no identifier filter.
  ...
  ///
  /// NPS documents only `relevanceScore` for things to do and answers other fields with HTTP 400.
  ...
  /// - Parameters:
  ///   - identifiers: Zero or more validated thing to do identifiers, sent as `id`.
  ...
  ///   - sort: Ordered criteria; NPS documents only `relevanceScore`.
  ...
  /// The things to do parameters for this page, omitting empty identifier, code, and sort arrays.

/// Filters, sorting, and pagination settings for the NPS photo gallery assets endpoint.
///
...
/// maximum is assumed.
///
/// Gallery identifiers are sent as `galleryId`. The live service returns a gallery's assets for
/// an uppercase gallery UUID, and the totals of a comma-separated list add up. It matches case
/// sensitively, so a lowercase UUID returns no assets, and an unknown UUID returns none either. A
/// value that is not UUID-shaped, such as `zzzz`, is silently ignored, and every asset comes
/// back. Asset identifiers sent as `id` behave the same way. Neither is validated here.
///
/// ```swift
/// let query = try PhotoGalleryAssetQuery(
///   galleryIdentifiers: [NPSIdentifier("1FFC7EF8-155D-4519-3ECC-B652E2E95E20")])
/// let endpoint = Endpoint.photoGalleryAssets(query: query)
/// ```
public struct PhotoGalleryAssetQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``PhotoGalleryAsset`` values.
  public typealias Item = PhotoGalleryAsset

  /// The photo gallery assets collection path.
  public static let path = "/multimedia/galleries/assets"

  /// Gallery identifiers in caller-supplied order, sent as `galleryId`; empty means no gallery
  /// filter.
  public let galleryIdentifiers: [NPSIdentifier]
  /// Asset identifiers in caller-supplied order, sent as `id`; empty means no identifier filter.
  ...
  /// fields such as `relevanceScore`. Fields are sent without validation.
  ...
  /// - Parameters:
  ///   - galleryIdentifiers: Zero or more gallery identifiers, sent as `galleryId`.
  ///   - identifiers: Zero or more asset identifiers, sent as `id`.
  ...
  ///   - sort: Ordered criteria naming photo gallery asset properties, such as `title`.
  ...
  public init(
    galleryIdentifiers: [NPSIdentifier] = [], identifiers: [NPSIdentifier] = [],
  ...
  private init(copy: Self, start: Int) {
    self.galleryIdentifiers = copy.galleryIdentifiers
  ...
  /// The photo gallery asset parameters for this page, omitting empty identifier, code, and sort
  /// arrays.
  public var queryItems: [NPSQueryItem] {
    var items: [NPSQueryItem] = []
    if !galleryIdentifiers.isEmpty {
      items.append(NPSQueryItem(name: "galleryId", values: galleryIdentifiers.map(\.rawValue)))
    }

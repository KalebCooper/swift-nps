...
/// ``ParkActivityParks/parks`` to the requested parks. The live endpoint ignores an identifier it
/// does not recognize rather than matching nothing.
///
/// ```swift
/// let query = try ParkActivityParksQuery(
///   parkCodes: [ParkCode("drto")], sort: [.descending("name")])
/// let endpoint = Endpoint.parkActivityParks(query: query)
/// ```
public struct ParkActivityParksQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkActivityParks`` values.
  public typealias Item = ParkActivityParks

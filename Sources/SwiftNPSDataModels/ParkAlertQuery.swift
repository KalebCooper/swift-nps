/// Filters and pagination settings for the NPS alerts endpoint.
///
/// Empty code arrays omit those parameters. Search text is preserved, including empty text. The
/// explicit defaults request 50 results starting at zero. No provider maximum is assumed. NPS
/// documents no sort parameter for alerts, so this query has none.
///
/// ```swift
/// let query = try ParkAlertQuery(parkCodes: [ParkCode("acad"), ParkCode("yell")])
/// let endpoint = Endpoint.parkAlerts(query: query)
/// ```
public struct ParkAlertQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkAlert`` values.
  public typealias Item = ParkAlert

  /// The alerts collection path.
  public static let path = "/alerts"
  ...
  /// The alerts parameters for this page, omitting empty code arrays.

/// Filters, sorting, and pagination settings for the NPS topic parks endpoint.
///
...
///
/// Park codes select the topics associated with those parks and also narrow each topic's
/// ``ParkTopicParks/parks`` to the requested parks. The live endpoint ignores an identifier it does
/// not recognize rather than matching nothing.
///
/// ```swift
/// let query = try ParkTopicParksQuery(parkCodes: [ParkCode("mamc")], sort: [.descending("name")])
/// let endpoint = Endpoint.parkTopicParks(query: query)
/// ```
public struct ParkTopicParksQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkTopicParks`` values.
  public typealias Item = ParkTopicParks

  /// The topic parks collection path.
  public static let path = "/topics/parks"

  /// Topic identifiers in caller-supplied order; empty means no identifier filter.
  ...
  /// - Parameters:
  ///   - identifiers: Zero or more validated topic identifiers, sent as `id`.
  ...
  ///   - sort: Ordered criteria naming topic properties, such as `name`.
  ...
  /// The topic parks parameters for this page, omitting empty identifier, code, and sort arrays.

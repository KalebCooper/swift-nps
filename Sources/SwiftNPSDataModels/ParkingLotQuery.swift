/// Filters, sorting, and pagination settings for the NPS parking lots endpoint.
///
...
///   parkCodes: [ParkCode("chsc")], sort: [.descending("name")])
/// let endpoint = Endpoint.parkingLots(query: query)
/// ```
public struct ParkingLotQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkingLot`` values.
  public typealias Item = ParkingLot

  /// The parking lots collection path.
  public static let path = "/parkinglots"
  ...
  ///
  /// The live endpoint sorts by `name` and `parkCode`, ascending or descending, and answers HTTP
  /// 400 for other fields such as `title` and `relevanceScore`. Fields are sent without
  /// validation.
  ...
  ///   - sort: Ordered criteria naming parking lot properties, such as `name`.
  ...
  /// The parking lot parameters for this page, omitting empty code and sort arrays.

/// Filters, sorting, and pagination settings for the NPS lesson plans endpoint.
///
...
/// maximum is assumed.
///
/// A recognized identifier selects that lesson plan, while the live endpoint ignores an identifier
/// it does not recognize and returns the unfiltered set rather than matching nothing. Park codes
/// select the lesson plans related to those parks without narrowing each plan's
/// ``LessonPlan/parks``.
///
/// ```swift
/// let query = try LessonPlanQuery(parkCodes: [ParkCode("tusk")], sort: [.descending("title")])
/// let endpoint = Endpoint.lessonPlans(query: query)
/// ```
public struct LessonPlanQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``LessonPlan`` values.
  public typealias Item = LessonPlan

  /// The lesson plans collection path.
  public static let path = "/lessonplans"

  /// Lesson plan identifiers in caller-supplied order; empty means no identifier filter.
  ...
  /// - Parameters:
  ///   - identifiers: Zero or more validated lesson plan identifiers, sent as `id`.
  ...
  ///   - sort: Ordered criteria naming lesson plan properties, such as `title`.
  ...
  /// The lesson plans parameters for this page, omitting empty identifier, code, and sort arrays.

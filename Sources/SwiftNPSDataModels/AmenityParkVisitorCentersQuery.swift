/// Filters, sorting, and pagination settings for the NPS amenity park visitor centers endpoint.
///
...
/// maximum is assumed. NPS documents no state parameter for `/amenities/parksvisitorcenters`, so this query has none.
///
/// Each page's `data` holds the provider's per-amenity groups, so ``Item`` is an array of
/// ``AmenityParkVisitorCenters``. The offset advances by the number of groups received, which the live service
/// counts against `limit`.
///
/// ```swift
/// let query = try AmenityParkVisitorCentersQuery(parkCodes: [ParkCode("acad")], sort: [.ascending("name")])
/// let endpoint = Endpoint.amenityParkVisitorCenters(query: query)
/// ```
public struct AmenityParkVisitorCentersQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of per-amenity groups of ``AmenityParkVisitorCenters`` values.
  public typealias Item = [AmenityParkVisitorCenters]

  /// The amenity park visitor centers collection path.
  public static let path = "/amenities/parksvisitorcenters"
  ...
  /// The amenity park visitor centers parameters for this page, omitting empty identifier, code, and
  /// sort arrays.

/// Filters, sorting, and pagination settings for the NPS news releases endpoint.
///
...
/// let endpoint = Endpoint.newsReleases(query: query)
/// ```
public struct NewsReleaseQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``NewsRelease`` values.
  public typealias Item = NewsRelease

  /// The news releases collection path.
  public static let path = "/newsreleases"
  ...
  ///
  /// The live endpoint sorts by `releaseDate` and `title` and answers HTTP 400 for other fields
  /// such as `relevanceScore`. Fields are sent without validation.
  ...
  ///   - sort: Ordered criteria naming news release properties, such as `releaseDate`.
  ...
  /// The news releases parameters for this page, omitting empty code and sort arrays.

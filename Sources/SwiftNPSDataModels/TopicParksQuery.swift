...
/// ``TopicParks/parks`` to the requested parks. The live endpoint ignores an identifier it does
/// not recognize rather than matching nothing.
///
/// ```swift
/// let query = try TopicParksQuery(parkCodes: [ParkCode("mamc")], sort: [.descending("name")])
/// let endpoint = Endpoint.topicParks(query: query)
/// ```
public struct TopicParksQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``TopicParks`` values.
  public typealias Item = TopicParks

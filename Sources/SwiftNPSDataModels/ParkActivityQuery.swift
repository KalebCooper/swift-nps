/// Filters, sorting, and pagination settings for the NPS activities endpoint.
///
...
///
/// Park codes narrow the returned activities to those offered at the requested parks. The live
/// endpoint ignores an identifier it does not recognize rather than matching nothing.
///
/// ```swift
/// let query = try ParkActivityQuery(parkCodes: [ParkCode("drto")], sort: [.descending("name")])
/// let endpoint = Endpoint.parkActivities(query: query)
/// ```
public struct ParkActivityQuery: NPSCollectionQuery {
  ...
  /// Each page decodes as a collection of ``ParkActivity`` values.
  public typealias Item = ParkActivity

  /// The activities collection path.
  public static let path = "/activities"
  ...
  /// The activities parameters for this page, omitting empty identifier, code, and sort arrays.

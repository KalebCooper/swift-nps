...
struct ActivityClientTests {
  ...
  func anEmptyActivitiesPageEndsIterationWithoutAnotherRequest(_ items: Bool) async throws {
    ...
    let query = try ActivityQuery(limit: 1, parkCodes: [ParkCode("zzzz")])
    if items {
      var activities: [Activity] = []
      for try await activity in client.activities(query: query) { activities.append(activity) }
      #expect(activities.isEmpty)
    } else {
      var pages: [NPSCollection<Activity>] = []
      for try await page in client.activityPages(query: query) { pages.append(page) }
  ...
  func activitiesItemIterationFetchesTheNextPageOnlyWhenNeeded() async throws {
    ...
    let sequence = try makeClient(transport).activities(query: makeQuery())
    let _: NPSItemSequence<Activity> = sequence
  ...
  func activitiesPagesAdvanceLazilyThroughTheRecordedPages() async throws {
    ...
    let sequence = try makeClient(transport).activityPages(query: makeQuery())
    let _: NPSPageSequence<Activity> = sequence
    var iterator = sequence.makeAsyncIterator()
    #expect(transport.requests.isEmpty)
    let firstPage = try await iterator.next()
    #expect(firstPage == (try JSONDecoder().decode(NPSCollection<Activity>.self, from: first)))
    #expect(transport.requests.count == 1)
    let lastPage = try await iterator.next()
    #expect(lastPage == (try JSONDecoder().decode(NPSCollection<Activity>.self, from: last)))
    #expect(transport.requests.map(\.request.path) == [Self.firstPath, Self.lastPath])
  ...
  func reusableActivitiesRequestsReturnTheSamePageAsTheirEndpoint() async throws {
    ...
    let reusable = try await client.value(for: .activities(query: query))
    let endpoint = try await client.send(.activities(query: query))
  ...
  private func makeQuery() throws -> ActivityQuery {
    try ActivityQuery(
      identifiers: [

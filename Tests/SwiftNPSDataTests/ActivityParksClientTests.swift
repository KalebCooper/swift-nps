...
struct ActivityParksClientTests {
  ...
  func anEmptyActivityParksPageEndsIterationWithoutAnotherRequest(_ items: Bool) async throws {
    ...
    let query = try ActivityParksQuery(limit: 1, parkCodes: [ParkCode("zzzz")])
    if items {
      var activities: [ActivityParks] = []
      for try await activity in client.activityParks(query: query) { activities.append(activity) }
      #expect(activities.isEmpty)
    } else {
      var pages: [NPSCollection<ActivityParks>] = []
      for try await page in client.activityParkPages(query: query) { pages.append(page) }
  ...
  func activityParksItemIterationFetchesTheNextPageOnlyWhenNeeded() async throws {
    ...
    let sequence = try makeClient(transport).activityParks(query: makeQuery())
    let _: NPSItemSequence<ActivityParks> = sequence
  ...
  func activityParksPagesAdvanceLazilyThroughTheRecordedPages() async throws {
    ...
    let sequence = try makeClient(transport).activityParkPages(query: makeQuery())
    let _: NPSPageSequence<ActivityParks> = sequence
    var iterator = sequence.makeAsyncIterator()
    #expect(transport.requests.isEmpty)
    let firstPage = try await iterator.next()
    #expect(
      firstPage == (try JSONDecoder().decode(NPSCollection<ActivityParks>.self, from: first)))
    #expect(transport.requests.count == 1)
    let lastPage = try await iterator.next()
    #expect(
      lastPage == (try JSONDecoder().decode(NPSCollection<ActivityParks>.self, from: last)))
    #expect(transport.requests.map(\.request.path) == [Self.firstPath, Self.lastPath])
  ...
  func reusableActivityParksRequestsReturnTheSamePageAsTheirEndpoint() async throws {
    ...
    let reusable = try await client.value(for: .activityParks(query: query))
    let endpoint = try await client.send(.activityParks(query: query))
  ...
  private func makeQuery() throws -> ActivityParksQuery {

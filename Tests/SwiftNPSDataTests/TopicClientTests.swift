...
struct TopicClientTests {
  ...
  func anEmptyTopicsPageEndsIterationWithoutAnotherRequest(_ items: Bool) async throws {
    ...
    let query = try TopicQuery(limit: 1, parkCodes: [ParkCode("zzzz")])
    if items {
      var topics: [Topic] = []
      for try await topic in client.topics(query: query) { topics.append(topic) }
      #expect(topics.isEmpty)
    } else {
      var pages: [NPSCollection<Topic>] = []
      for try await page in client.topicPages(query: query) { pages.append(page) }
  ...
  func topicsItemIterationFetchesTheNextPageOnlyWhenNeeded() async throws {
    ...
    let sequence = try makeClient(transport).topics(query: makeQuery())
    let _: NPSItemSequence<Topic> = sequence
  ...
  func topicsPagesAdvanceLazilyThroughTheRecordedPages() async throws {
    ...
    let sequence = try makeClient(transport).topicPages(query: makeQuery())
    let _: NPSPageSequence<Topic> = sequence
    var iterator = sequence.makeAsyncIterator()
    #expect(transport.requests.isEmpty)
    let firstPage = try await iterator.next()
    #expect(firstPage == (try JSONDecoder().decode(NPSCollection<Topic>.self, from: first)))
    #expect(transport.requests.count == 1)
    let lastPage = try await iterator.next()
    #expect(lastPage == (try JSONDecoder().decode(NPSCollection<Topic>.self, from: last)))
    #expect(transport.requests.map(\.request.path) == [Self.firstPath, Self.lastPath])
  ...
  func reusableTopicsRequestsReturnTheSamePageAsTheirEndpoint() async throws {
    ...
    let reusable = try await client.value(for: .topics(query: query))
    let endpoint = try await client.send(.topics(query: query))
  ...
  private func makeQuery() throws -> TopicQuery {
    try TopicQuery(
      identifiers: [

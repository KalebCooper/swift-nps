...
struct ParkTopicParksClientTests {
  ...
  func anEmptyTopicParksPageEndsIterationWithoutAnotherRequest(_ items: Bool) async throws {
    ...
    let query = try ParkTopicParksQuery(limit: 1, parkCodes: [ParkCode("zzzz")])
    if items {
      var topics: [ParkTopicParks] = []
      for try await topic in client.parkTopicParks(query: query) { topics.append(topic) }
      #expect(topics.isEmpty)
    } else {
      var pages: [NPSCollection<ParkTopicParks>] = []
      for try await page in client.parkTopicParkPages(query: query) { pages.append(page) }
  ...
  func topicParksItemIterationFetchesTheNextPageOnlyWhenNeeded() async throws {
    ...
    let sequence = try makeClient(transport).parkTopicParks(query: makeQuery())
    let _: NPSItemSequence<ParkTopicParks> = sequence
  ...
  func topicParksPagesAdvanceLazilyThroughTheRecordedPages() async throws {
    ...
    let sequence = try makeClient(transport).parkTopicParkPages(query: makeQuery())
    let _: NPSPageSequence<ParkTopicParks> = sequence
    var iterator = sequence.makeAsyncIterator()
    #expect(transport.requests.isEmpty)
    let firstPage = try await iterator.next()
    #expect(
      firstPage == (try JSONDecoder().decode(NPSCollection<ParkTopicParks>.self, from: first)))
    #expect(transport.requests.count == 1)
    let lastPage = try await iterator.next()
    #expect(lastPage == (try JSONDecoder().decode(NPSCollection<ParkTopicParks>.self, from: last)))
    #expect(transport.requests.map(\.request.path) == [Self.firstPath, Self.lastPath])
  ...
  func reusableTopicParksRequestsReturnTheSamePageAsTheirEndpoint() async throws {
    ...
    let reusable = try await client.value(for: .parkTopicParks(query: query))
    let endpoint = try await client.send(.parkTopicParks(query: query))
  ...
  private func makeQuery() throws -> ParkTopicParksQuery {

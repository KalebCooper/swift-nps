...
struct ParkTopicParksTests {
  ...
  func theRecordedSearchPageDecodesEveryTopicAndParkFieldAsSent() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkTopicParks>.self, from: Fixture.topicParksSearch.data())
    #expect(page.total == "1")
    #expect(page.limit == "2")
    #expect(page.start == "0")
  ...
  func theRecordedPagesNarrowEachTopicToTheRequestedPark() throws {
    ...
    let last = try JSONDecoder().decode(
      NPSCollection<ParkTopicParks>.self, from: Fixture.topicParksPageLast.data())
  ...
  func theRecordedEmptyPageDecodesWithNoTopics() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkTopicParks>.self, from: Fixture.topicParksEmpty.data())
    #expect(page.total == "0")
  ...
  func aMissingOrNullParksArrayDecodesAsNil(_ json: String) throws {
    let topic = try JSONDecoder().decode(ParkTopicParks.self, from: Data(json.utf8))
    #expect(topic.id == "A1")

...
struct TopicTests {
  ...
  func theRecordedSearchPageDecodesEveryTopicFieldAsSent() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<Topic>.self, from: Fixture.topicsSearch.data())
    #expect(page.total == "1")
    #expect(page.limit == "2")
    #expect(page.start == "0")
  ...
  func theRecordedPagesAdvanceThroughTheRequestedIdentifiers() throws {
    ...
    let last = try JSONDecoder().decode(
      NPSCollection<Topic>.self, from: Fixture.topicsPageLast.data())
  ...
  func theRecordedEmptyPageDecodesWithNoTopics() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<Topic>.self, from: Fixture.topicsEmpty.data())
    #expect(page.total == "0")
  ...
  func aTopicDecodesItsIdentifierAndNameAsSent() throws {
    let topic = try JSONDecoder().decode(
      Topic.self, from: Data(#"{"id":"A1","name":"Wildlife"}"#.utf8))
    #expect(topic.id == "A1")

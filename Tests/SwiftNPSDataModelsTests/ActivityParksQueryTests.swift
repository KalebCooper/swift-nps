...
struct ActivityParksQueryTests {
  ...
  func allDocumentedActivityParksParametersPreserveCallerValues() throws {
    ...
    #expect(
      Endpoint.activityParks(query: query).path
        == "/activities/parks?id=B2,A1&limit=2&parkCode=DRTO,cwdw&q=a%20%2B%26%2F%23%3F%C3%A9"
        + "&sort=-name,id&start=3")
    #expect(Endpoint.collection(query) == Endpoint.activityParks(query: query))
  }
  ...
  func defaultActivityParksQueriesExplicitlyRequestFiftyResultsAtZero() throws {
    #expect(
      try Endpoint.activityParks(query: ActivityParksQuery()).path
        == "/activities/parks?limit=50&start=0")
    #expect(
      try Endpoint.activityParks(query: ActivityParksQuery(searchText: "")).path
  ...
  func emptyActivityParksIdentifierCodeAndSortArraysOmitTheirParameters() throws {
    let query = try ActivityParksQuery(identifiers: [], limit: 1, parkCodes: [], sort: [])
    #expect(Endpoint.activityParks(query: query).path == "/activities/parks?limit=1&start=0")
  ...
  func invalidActivityParksQueryPaginationIsRejected() {
    #expect(throws: ActivityParksQuery.ValidationError.invalidLimit) {
      try ActivityParksQuery(limit: 0)
    }
    #expect(throws: ActivityParksQuery.ValidationError.invalidLimit) {
      try ActivityParksQuery(limit: -1)
    }
    #expect(throws: ActivityParksQuery.ValidationError.invalidStart) {
      try ActivityParksQuery(start: -1)
    }
  }
  ...
  func activityParksRequestsResolveAsCollectionsOfTheSameQuery() throws {
    let query = try ActivityParksQuery(limit: 1, parkCodes: [ParkCode("drto")])
    let request = NPSDataRequest.activityParks(query: query)
    let _: NPSDataRequest<NPSCollection<ActivityParks>> = request
    #expect(request.resolution == .collection(NPSCollectionResolution(query)))
    #expect(Set([request, .activityParks(query: query)]).count == 1)
    #expect(request != .activityParks(query: try ActivityParksQuery(limit: 2)))
    guard case .collection(let resolution) = request.resolution else {
      Issue.record("A query request must resolve as a collection.")
      return
    }
    #expect(resolution.endpoint == Endpoint.activityParks(query: query))
    #expect(resolution.query as? ActivityParksQuery == query)
  }
  ...
  func anActivityParksQueryAdvancesByTheReturnedItemCountAndKeepsItsFilters() throws {
    ...
    #expect(
      Endpoint.activityParks(query: next).path

...
struct ParkActivityParksTests {
  ...
  func theRecordedSearchPageDecodesEveryActivityAndParkFieldAsSent() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkActivityParks>.self, from: Fixture.activityParksSearch.data())
    #expect(page.total == "1")
    #expect(page.limit == "2")
    #expect(page.start == "0")
  ...
  func theRecordedPagesNarrowEachActivityToTheRequestedPark() throws {
    ...
    let last = try JSONDecoder().decode(
      NPSCollection<ParkActivityParks>.self, from: Fixture.activityParksPageLast.data())
  ...
  func theRecordedEmptyPageDecodesWithNoActivities() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkActivityParks>.self, from: Fixture.activityParksEmpty.data())
    #expect(page.total == "0")
  ...
  func aMissingOrNullParksArrayDecodesAsNil(_ json: String) throws {
    let activity = try JSONDecoder().decode(ParkActivityParks.self, from: Data(json.utf8))

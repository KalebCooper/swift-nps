...
struct ParkActivityTests {
  ...
  func theRecordedSearchPageDecodesEveryActivityFieldAsSent() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkActivity>.self, from: Fixture.activitiesSearch.data())
    #expect(page.total == "1")
    #expect(page.limit == "2")
    #expect(page.start == "0")
  ...
  func theRecordedPagesAdvanceThroughTheRequestedIdentifiers() throws {
    ...
    let last = try JSONDecoder().decode(
      NPSCollection<ParkActivity>.self, from: Fixture.activitiesPageLast.data())
  ...
  func theRecordedEmptyPageDecodesWithNoActivities() throws {
    let page = try JSONDecoder().decode(
      NPSCollection<ParkActivity>.self, from: Fixture.activitiesEmpty.data())
    #expect(page.total == "0")
  ...
  func anActivityDecodesItsIdentifierAndNameAsSent() throws {
    let activity = try JSONDecoder().decode(
      ParkActivity.self, from: Data(#"{"id":"A1","name":"Hiking"}"#.utf8))
